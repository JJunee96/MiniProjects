...
class qtApp(QMainWindow):
    ...
    def btnSaveClicked(self): # 저장
        ...
        if fullName == '' or phoneNum == '':
            QMessageBox.warning(self, '주의', '이름과 스마트폰번호를 입력하세요!')
            return # 진행불가
        else:
            self.conn = pymysql.connect(host= 'localhost', user = 'root', password='12345'
                                    , db='miniproject', charset = 'utf8')
            ...
            # 네개 변수값 받아서 INSERT 쿼리문 만들기
                query = '''INSERT INTO addressbook(FullName,PhonNum,Email,Address)
                            	VALUES (%s, %s, %s, %s);'''
            else:
    ...
    def makeTable(self, rows) :
        ...
        self.tblAddress.setColumnWidth(0, 20) # 1. 번호는 숨김
        self.tblAddress.setColumnWidth(1, 70) # 이름 열 79
        ...
        self.tblAddress.setEditTriggers(QAbstractItemView.NoEditTriggers) # 1. 컬럼수정 금지 
